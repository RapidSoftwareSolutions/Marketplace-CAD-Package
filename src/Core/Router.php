...
class Router
{
    ...
    )
    {
        ...
        $this->supportedMethods = ['POST', 'POST-NO-CACHE'];
    }
    ...
    private function setRoute($block)
    {
        ...
        $this->klein->respond('POST', $routePath, function()use($param, $blockName, $blockCustom, $method){
            ...
            if(isset($blockCustom['custom'])&&$blockCustom['custom'] == true){
                ...
                // Remove $apiKey from params
                if(isset($sendBody['apiKey'])&&strlen($sendBody['apiKey'])>0){
                    $apiKey = $sendBody['apiKey'];
                    unset($sendBody['apiKey']);
                }else{
                    $apiKey = false;
                }
            }

            // Prepare $sendBody for 'multipart'
            $multipartSendBody = [];
            foreach($sendBody as $name => $contents){
                $multipartSendBody[] = [
                    'name' => $name,
                    'contents' => $contents,
                ];
            }
            $sendBody = $multipartSendBody;

            // Make request
            $result = $this->httpRequest($vendorUrl, $method, $sendBody, $apiKey);
            echo json_encode($result);
            exit(200);
        });
    ...
    protected function httpRequest($url, $method, $sendBody, $apiKey)
    {
        ...
        try {
            // Setup client
            $clientSetup['headers']['api-key'] = $apiKey;

            if($method == 'POST-NO-CACHE'){
                $clientSetup['headers']['cache-control'] = 'no-cache';
                $method = 'POST';
            }
